#include "veins-mmwave/mac/FmlAPMac2.h"

using namespace veins;

using std::unique_ptr;

Define_Module(veins::FmlAPMac2);

void veins::FmlAPMac2::initialize(int stage) {
...
void veins::FmlAPMac2::handleUpperMsg(cMessage* msg) {
...
void veins::FmlAPMac2::handleServiceRequest(FmlServiceRequest* request) {
...
uint32_t veins::FmlAPMac2::getSelectedBeamByFMLAlgorithm(uint32_t doAIndex) {
...
void veins::FmlAPMac2::updateFMLStep(FmlServiceFeedback* feedback) {
...
void veins::FmlAPMac2::updateFMLStep() {
...
double veins::FmlAPMac2::getFactorControlFunction() {
    return factorControlFunctionValue;
}

double veins::FmlAPMac2::getControlFunction(uint32_t z, uint32_t t) {
    return pow(t, z) * log(t);
}

void veins::FmlAPMac2::initializeFML() {
...
void veins::FmlAPMac2::handleSelfMsg(cMessage* msg) {
    ...
    if(msg == selectingANewBeam) {
        ...
        if(evaluationServingSTA == vehicleTriggeringBeamSelection) {
            uint32_t receivingBeamID = vehicleTriggeringBeamSelectionPhy80211ad->getBestBeamByTheOtherPosition(phy11ad->getCurrentPosition());
            double rxPower = vehicleTriggeringBeamSelectionPhy80211ad->getRxPowerToRSUAt(simTime(), phy11ad->getCurrentPosition(), phy11ad->getCurrentOrientation(), selectedBeam, receivingBeamID);
        ...
        for(auto const& it : availableCars) {
            ...
            double rxPower = staPhyMmWave->getRxPowerToRSUAt(simTime(), rsuPosition, rsuOrientation, selectedBeam, bestBeamTowardRSU);
...
bool veins::FmlAPMac2::serveTheNewVehicle() {
...
int32_t veins::FmlAPMac2::getReceivingSectorIDForOracle(
...
void veins::FmlAPMac2::performBTI() {
...
bool veins::FmlAPMac2::doesServingVehicleLeaveTheBeam(Mac11adToPhy11adInterface* phyMmWave, LAddress::L2Type vehicleMAC, double sensitivity) {
    Coord rsuPosition = phy11ad->getCurrentPosition();
    Coord rsuOrientation = phy11ad->getCurrentOrientation();

    //Estimate for 10 more seconds
    uint32_t receivingBeamID = phyMmWave->getBestBeamByTheOtherPosition(phy11ad->getCurrentPosition());
...
//        uint32_t bestBeamTowardRSU = phyMmWave->getBestBeamByTheOtherPositionAt(rsuPosition, i);
        double rxPower = phyMmWave->getRxPowerToRSUAt(i, rsuPosition, rsuOrientation, selectedBeam, receivingBeamID);
...
void veins::FmlAPMac2::whenServingCarLeaveTheBeam(bool teleported) {
...
void veins::FmlAPMac2::handleAck(const MacAck* ack) {
...
void veins::FmlAPMac2::pumpEvaluatePacketToQueue() {
    if (estimateRX->isScheduled())
